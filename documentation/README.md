...
| `/.claude/skills` | Claude Code skills for this project — this is the path Claude Code actually scans to make a skill available as a `/` slash command, and the actual location of every skill's files. |
...
| `/skills` | A visible, non-hidden entry point for browsing — each skill here is a symlink into `/.claude/skills`, not a separate copy, so there's nothing to keep in sync. |
| `/skills/thesis-topic-assessment` | Symlink to `/.claude/skills/thesis-topic-assessment`. |
